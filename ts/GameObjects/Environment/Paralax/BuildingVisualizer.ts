...
export default class BuildingVisualizer extends Phaser.Group
{
    ...
    constructor (game: Phaser.Game, maxWidth: number, maxHeight: number) {
        ...
        this._maskGraphic.drawRect(0, -this.maxHeight, this.maxWidth, maxHeight);
    ...
    private setUp(): void {
        //bottom half
        this._backGlow = new Phaser.Sprite(this.game, 0, 0, Atlases.Interface, 'Purple_glow_utopia');
        ...
        //top half
        this._topBackGlow = new Phaser.Sprite(this.game, 0, 0, Atlases.Interface, 'Purple_glow_utopia');
        this._topBackGlow.anchor.set(0, 1);
        this._topHalf.addChild(this._topBackGlow);

        this.setUpBuildings(true);

        this._topGlow = new Phaser.Sprite(this.game, 0, 0, Atlases.Interface, 'Upper_glow_utopia');
    ...
    private setUpBuildings(top: boolean): void {
        let pos: number = 0;
        while (pos < this.maxWidth) {
            let index: number =  Math.ceil(Math.random() * 14);
            let building: Phaser.Sprite = new Phaser.Sprite(this.game, pos, 0, Atlases.Interface, 'Background_Building_Blue_' + (index > 9 ? index : ('0' + index)));
            building.anchor.set(0);
            building.scale.set(.5);
    ...
    private deactivateHalf(): void {
        if (!this._topIsActive) {
            this.game.add.tween(this._topHalf).to({y: -this.maxHeight * 2}, this._switchTime, Phaser.Easing.Cubic.InOut, true);
            this.game.add.tween(this._bottomHalf).to({y: 0}, this._switchTime, Phaser.Easing.Cubic.InOut, true);
        } else {
            this.game.add.tween(this._topHalf).to({y: -this.maxHeight}, this._switchTime, Phaser.Easing.Cubic.InOut, true);
            this.game.add.tween(this._bottomHalf).to({y: this.maxHeight}, this._switchTime, Phaser.Easing.Cubic.InOut, true);
        }
    }
